...
def get_absolute_action(env, action):
    ...
    if goal_ori_quat[3] < 0:  # enforce w >= 0 for consistent short-path axis-angle
        goal_ori_quat = -goal_ori_quat
...
def main(args):
    ...
    if args.resume and os.path.exists(metainfo_json_out_path):
        with open(metainfo_json_out_path, "r") as f:
            metainfo_json_dict = json.load(f)
        print(f"Resuming: loaded existing metainfo from {metainfo_json_out_path}")
    else:
        metainfo_json_dict = {}
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        # Skip tasks whose output file already exists when resuming
        new_data_path = os.path.join(args.libero_target_dir, f"{task.name}_demo.hdf5")
        if args.resume and os.path.exists(new_data_path):
            print(f"Skipping task '{task.name}' (already saved at {new_data_path})")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume", action="store_true",
                        help="Resume from existing data in --libero_target_dir, skipping tasks whose output HDF5 already exists.")
